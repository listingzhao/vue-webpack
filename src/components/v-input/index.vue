<template lang="html">
  <div class="v-x-input weui-cell" :class="{'weui-cell_warn': !valid}">
    ...
    <div class="weui-cell__ft">
      ...
      <icon class="v-input-icon" type="warn" :title="!valid ? firstError : ''" v-show="!novalidate && !equalWith && ((touched && !valid && firstError) || (forceShowError && !valid && firstError))"></icon>
      <icon class="v-input-icon" type="warn" v-if="!novalidate && hasLengthEqual && dirty && equalWith && !valid"></icon>
      <icon type="success" v-show="!novalidate && equalWith && equalWith === currentValue && valid"></icon>

      <icon type="success" class="v-input-icon" v-show="novalidate && iconType === 'success'"></icon>
      <icon type="warn" class="v-input-icon" v-show="novalidate && iconType === 'error'"></icon>
...
@import '../../styles/weui/widget/weui-cell/weui-form/weui-vcode';

.v-input-icon.v-input-icon {
  font-size: 21px;
}
.v-input-icon.weui-icon-warn:before, .v-input-icon.weui-icon-success:before {
  font-size: 21px;
}
.v-x-input .weui-icon {
  padding-left: 5px;
}
.v-x-input.weui-cell_vcode {
  padding-top: 0;
  padding-right: 0;
  padding-bottom: 0;
}
